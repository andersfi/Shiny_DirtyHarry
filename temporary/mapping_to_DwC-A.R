...
# Read data 
#------------------------------------------------------------------------------
testdatafolder <- "/testdata_per-arne/"
file <- paste(getwd(),testdatafolder,"input_data_example.xlsx",sep="")
inndata_wb <- loadWorkbook(file, create = F)
file <- paste(getwd(),testdatafolder,"input_data_example.xlsx",sep="")
mapping <- read.csv(paste(getwd(),testdatafolder,"input_table_mapping.csv",sep=""),stringsAsFactors=FALSE,strip.white=TRUE)
translation_table <- read.csv(paste(getwd(),testdatafolder,"input_table_translation.csv",sep=""),stringsAsFactors=FALSE,strip.white=TRUE)
...
#------------------------------------------------------------------------------
# Flatten out input data in case of multisheet workbooks
#------------------------------------------------------------------------------

inndata <- get_data_from_xlsx_and_flatten_out(inndata_wb,mapping)
